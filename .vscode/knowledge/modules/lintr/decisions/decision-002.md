...
- Phase 6 驗證後，RTK analytics 只作觀察用途：`rtk gain` 追蹤 savings、`rtk discover` 找 missed opportunities、`rtk session` 查 session data；analytics 不取代 raw/canonical command 的錯誤判讀。
- Phase 6 後觀察一週；若發現 RTK 壓縮造成漏看錯誤、誤判或錯誤重試，先取回完整輸出，再新增或更新 operational trap。
...
Phase 6 驗證顯示 canonical `rebuild` / `finish-check` 可在 WSL workspace 內通過，RTK analytics 命令也可執行；目前尚未觀察到因 RTK 壓縮造成漏看錯誤，因此不新增 operational trap。
